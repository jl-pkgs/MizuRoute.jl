...
len=zeros(nreach); slope=zeros(nreach); width=zeros(nreach)
for (i,line) in enumerate(lines)
    x=split(line,','); rid[i]=parse(Int,x[1]); down[i]=parse(Int,x[2]); len[i]=parse(Float64,x[3]); slope[i]=parse(Float64,x[4]); width[i]=parse(Float64,x[5])
end
function read_f32(path,n)
    v=Vector{Float32}(undef,n); open(path,"r") do io; read!(io,v); end; v
end
raw=read_f32(joinpath(work,"qlat.f32"),nreach*ntime)
qlat=reshape(Float64.(raw),nreach,ntime)
...
methods = [("irf",IRF()), ("kwt",LagrangianKWT())]
for (name,method) in methods
    @info "Cameo routing" method=name nreach ntime
    model=RoutingModel(method,net,p;dt=dt)
